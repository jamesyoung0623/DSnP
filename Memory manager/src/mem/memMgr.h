...
#define toSizeT(t)       ((t%SIZE_T)>(0)?(t-(t%SIZE_T)+SIZE_T):(t))
//
...
class MemBlock
{
    ...
    bool getMem(size_t t, T*& ret) {
        if(t%SIZE_T != 0){t = toSizeT(t);} 
        if(_ptr + t > _end){ret = reinterpret_cast<T*>(_ptr);return false;}
        else{ret = reinterpret_cast<T*>(_ptr); _ptr = _ptr + t;return true;}
    }
...
class MemRecycleList
{
    ...
    void  pushFront(T* p){ 
            T** n = reinterpret_cast<T**>(p);
            * n = reinterpret_cast<T*>(_first);
            _first = reinterpret_cast<T*>(p);
    }
...
private:
    ...
    MemRecycleList<T>* getMemRecycleList(size_t n) {
          ...
          while(true)
          {
                if(p->getArrSize() == n){return p;}
                else
                {
                    if(p->getNextList() == 0)
                    {
                        MemRecycleList<T>* d = new MemRecycleList<T>(n); 
                        p->_nextList = d;
                        return p->_nextList;
                    }
                    else
    ...
    T* getMem(size_t t) {
        ...
        try { if(t > _blockSize) {throw bad_alloc();} } 
        catch (bad_alloc) 
        {  
            cerr << "Requested memory (" << t << ") is greater than block size" << "(" << _blockSize << "). " 
            << "Exception raised...\n"; 
            throw bad_alloc();
        }
        ...
        size_t nn ;
        if(t > toSizeT(S)){nn = t/S;}else{nn = 0;}
        if(nn < R_SIZE)
        {
            MemRecycleList<T>* list = &_recycleList[nn];
            ret = list->popFront();
            if(ret != 0)
            {
                #ifdef MEM_DEBUG
                cout << "Recycled from _recycleList[" << nn << "]..." << ret << endl;
                #endif // MEM_DEBUG
                return ret;
            }
        }
        ...
        if(_activeBlock->getMem(n,ret) == false)
        {
            size_t rn = _activeBlock->getRemainSize();
            if(rn > toSizeT(S)){rn = (rn-8)/S;}
            else if(rn < toSizeT(S)){rn = 0;}
            if(rn != 0)
            {  
            ...
            MemBlock<T>* p = new MemBlock<T>(0, _blockSize);
            _activeBlock->_nextBlock = p;
            _activeBlock = p;
            _numblocks++;
            _activeBlock->getMem(n,ret);
            #ifdef MEM_DEBUG
