...
CmdParser::readCmdInt(istream& istr)
{
   ...
   while (1) {
      ...
      switch (pch) {
         ...
         case PG_UP_KEY      : if(_historyIdx - PG_OFFSET < 0 && _historyIdx > 0)
                                {
                                    moveToHistory(0);
                                    break;
                                }
         ...
         case PG_DOWN_KEY    : if(_historyIdx + PG_OFFSET >= _history.size() && _historyIdx < _history.size()-1)
                                {
                                    moveToHistory(_history.size()-1);
                                    break;
                                }
                               else
                                {
         ...
         case TAB_KEY        : if(_readBufEnd == _readBufPtr)
                                {
                                    for(int i = 0;i<TAB_POSITION;i++){insertChar(' ');}break;
                                }
                                else
                                {
                                    for(int i = 0;i<TAB_POSITION/4;i++){insertChar(' ');}break;
                                }
...
CmdParser::insertChar(char ch, int repeat)
{
    ...
    if(_readBufEnd == _readBufPtr)
    {
        ...
        *(_readBufPtr) = ch;
        ...
        for(int i=0;i<(_readBufEnd-_readBufPtr);i++)
        {
            *(_readBufEnd+repeat-1-i) = *(_readBufEnd-1-i);
        }
...
{
    if(0<=index && index <_history.size())
    {
...
CmdParser::addHistory()
{
    ...
    for(int i = 0;i<(_readBufEnd-_readBuf);i++)
    {
        if(int(*(_readBufEnd-i-1)) != 32)
        {
            back = _readBufEnd-_readBuf-i-1;
            break;
        }
    }
    if(front != -1 && back != -1){
