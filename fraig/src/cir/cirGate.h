...
public:
    ...
    void addfout(CirGate* g){_foutList.push_back(g);}
    void setname(string s){_name = s;}
    void removefin(int i){_finList[i] = NULL;}
    void removefout(int i){_foutList.erase(_foutList.begin()+i);}
    void changefin(int i,CirGate* g,bool j)
    {
        _finList[i] = g;
        _input[i] = g->getid()*2;
        if(_phase[i] && j){_phase[i] = false;}
        else if(!_phase[i] && j){_phase[i] = true;}
        if(_phase[i])_input[i]++;
    }
    bool _isdfs = false;
